(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    // iterate over the collection
    _.each(collection, function(value) {
      // if an accumulator is provided
      if (accumulator !== undefined) {
        // set accumulator to the return value of calling iterator with the accumulator and current element of the collection
        var container = iterator(accumulator, value);
        if (container === undefined) {
          return;
        }
        accumulator = container;
      // if an accumulator is not provided
      } else {
        // set accumulator to first item of collection
        accumulator = value;
      }
    });
    return accumulator;
  ...
  _.every = function(collection, iterator) {
    ...
    iterator = iterator || _.identity;
    // return call to reduce function
    return _.reduce(collection, function(isTrue, currentItem) {
      // return accumulator
      return isTrue && !!iterator(currentItem);
    }, true);
  ...
  _.extend = function(obj) {
    ...
    for (var i = 1; i < arguments.length; i++) {
      ...
      for (var key in arguments[i]) {
        // add current property to object
  ...
  _.delay = function(func, wait) {
    // we're gonna use setTimeout with the function and wait time
    var args = Array.prototype.slice.call(arguments);
    var actualArguments = args.slice(2);
    setTimeout(function () {
      func.apply(this, actualArguments);
    }, wait);
  };
  ...
  _.shuffle = function(array) {
    ...
    while (copy.length > 0) {
      // generate a random index between 0 and length of copy
      var randomIndex = Math.floor(Math.random() * copy.length);
      // add element from the copy at the random index to the results array
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    if (typeof functionOrKey === 'function') {
      _.each(collection, function(value) {
        output.push(functionOrKey.apply(value, args));
      });
    } else {
      _.each(collection, function(value) {
        output.push(value[functionOrKey].apply(value, args));
      });
    }
  ...
  _.zip = function() {
    ...
    for (var i = 0; i < arguments[0].length; i++) {
      ...
      var container = [];
      // add element of longer array to container array
      for (var j = 0; j < arguments.length; j++) {
      // add element of shorter array to container array
        container.push(arguments[j][i]);
      }
